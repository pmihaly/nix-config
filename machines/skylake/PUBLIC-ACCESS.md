...
| `pinchflat.skylake.mihaly.codes`    | pinchflat (YouTube media manager)  | downloads media into `/persist/opt/skylake-storage/Public/yt`, the `yt` directory inside the public copyparty share — so downloaded media is served alongside it at `https://files.skylake.mihaly.codes/yt/`. UI behind HTTP basic auth (see the `server/pinchflat` agenix secret). Native nixpkgs `services.pinchflat` (bundled yt-dlp). See `modules/nixos/pinchflat/` |
