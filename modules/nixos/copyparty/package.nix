# Vendored from 9001/copyparty contrib/package/nix/copyparty/default.nix
# (v1.20.20) so we can apply patches (see ./patches/) — the upstream
# package has no `patches` argument. Diffs vs upstream are kept minimal
# (the unstable/git branch is dropped in favor of the PyPI sdist pin);
# rebase this file when bumping pin.json.
{
  lib,
  ...
  # source patches, applied by stdenv before the build
  patches ? [ ],
...
let
  ...
  # NOTE: the unstable (git source) branch is deliberately removed — this
